"""
routes/auth_routes.py — compatibility shim (Slice 2, #architecture-runtime-inventory).

The real implementation moved to ``routes/auth/routes.py``. This module is
aliased to that module object (via ``sys.modules``) rather than copying its
names, so ``from routes.auth_routes import X``, ``import routes.auth_routes
as X``, and ``mock.patch("routes.auth_routes.X", ...)`` all keep working
...
from routes.auth import routes as _real
