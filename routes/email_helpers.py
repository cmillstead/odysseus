"""
routes/email_helpers.py — compatibility shim (Slice 2, #architecture-runtime-inventory).

The real implementation moved to ``routes/email/helpers.py``. This module is
aliased to that module object (via ``sys.modules``) rather than copying its
names, so ``from routes.email_helpers import X``, ``import
routes.email_helpers as X``, and ``mock.patch("routes.email_helpers.X", ...)``
...
from routes.email import helpers as _real
