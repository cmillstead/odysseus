"""
routes/document_helpers.py — compatibility shim (Slice 2, #architecture-runtime-inventory).

The real implementation moved to ``routes/document/helpers.py``. This module is
aliased to that module object (via ``sys.modules``) rather than copying its
names, so ``from routes.document_helpers import X``, ``import
routes.document_helpers as X``, and ``mock.patch("routes.document_helpers.X", ...)``
...
from routes.document import helpers as _real
