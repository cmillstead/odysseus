"""
routes/document_routes.py — compatibility shim (Slice 2, #architecture-runtime-inventory).

The real implementation moved to ``routes/document/routes.py``. This module is
aliased to that module object (via ``sys.modules``) rather than copying its
names, so ``from routes.document_routes import X``, ``import
routes.document_routes as X``, and ``mock.patch("routes.document_routes.X", ...)``
...
from routes.document import routes as _real
