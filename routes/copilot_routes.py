"""
routes/copilot_routes.py — compatibility shim (Slice 2, #architecture-runtime-inventory).

The real implementation moved to ``routes/model/copilot.py``. This module is
aliased to that module object (via ``sys.modules``) rather than copying its
names, so ``from routes.copilot_routes import X``, ``import
routes.copilot_routes as X``, and ``mock.patch("routes.copilot_routes.X",
...
from routes.model import copilot as _real
