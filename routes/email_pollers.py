"""
routes/email_pollers.py — compatibility shim (Slice 2, #architecture-runtime-inventory).

The real implementation moved to ``routes/email/pollers.py``. This module is
aliased to that module object (via ``sys.modules``) rather than copying its
names, so ``from routes.email_pollers import X``, ``import
routes.email_pollers as X``, and ``mock.patch("routes.email_pollers.X", ...)``
...
from routes.email import pollers as _real
